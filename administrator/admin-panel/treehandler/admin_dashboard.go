...
type AdminDashboardData struct {
	TreeCount      int
	TotalTreeCount int // ← already added, good!
	VolunteerCount int
	SiteCount      int
	CurrentTime    string
	Trees          []TreeItem
...
func AdminDashboardHandler(w http.ResponseWriter, r *http.Request) {
	...
	if startDateStr != "" && endDateStr != "" {
		...
		endDate = endDate.Add(24 * time.Hour) // include full end date
	...
	// 👥 Fetch volunteers
	// 🔁 Remove the volunteers fetch – we’ll extract unique volunteer names from the tree data
	...
	for id, raw := range trees {
		totalTreeCount++ // Count every tree, filtered or not
		...
		if dateFilterEnabled && (parsedTime.Before(startDate) || parsedTime.After(endDate)) {
			continue
		}
	...
	data := AdminDashboardData{
		...
		TotalTreeCount: totalTreeCount, // ✅ NEW
		VolunteerCount: len(volunteerSet),
	...
	tmpl, err = tmpl.ParseFiles("templates/admin_dashboard.html")
	if err != nil {
